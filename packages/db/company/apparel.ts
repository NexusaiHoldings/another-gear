...
CREATE TABLE IF NOT EXISTS apparel_stories (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CHECK (published_at IS NULL OR consent_record_id IS NOT NULL)
);
`;
...
export const APPAREL_FK_CONSTRAINTS_DDL = `
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'apparel_stories_consent_record_id_fkey'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'apparel_stories_consent_record_id_key'
    ) THEN
        ALTER TABLE apparel_stories
        ADD CONSTRAINT apparel_stories_consent_record_id_key
