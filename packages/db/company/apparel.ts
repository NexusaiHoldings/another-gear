...
CREATE TABLE IF NOT EXISTS apparel_stories (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT apparel_stories_publication_consent_chk CHECK (published_at IS NULL OR consent_record_id IS NOT NULL)
);
...
CREATE TABLE IF NOT EXISTS apparel_consent_records (
    ...
    story_id UUID NOT NULL UNIQUE REFERENCES apparel_stories(id) ON DELETE CASCADE,
    release_document_url TEXT NOT NULL,
    signed_at TIMESTAMPTZ,
    ip_address INET,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT apparel_consent_records_subject_story_unique UNIQUE (subject_id, story_id)
);
...
BEGIN
    ALTER TABLE apparel_stories
        ADD CONSTRAINT apparel_stories_consent_record_fk
